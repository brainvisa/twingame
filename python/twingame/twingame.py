...
__version__ = '1.0.0'  # FIXME: get this from pyproject.toml: how ?
...
class TwinGame(Qt.QMainWindow):
    ...
    @staticmethod
    def get_share_path():
        mypath = osp.dirname(osp.dirname(__file__))
        if mypath in ('dist-packages', 'site-packages'):
            # user /system install
            ver = '.'.join(__version__.split('.')[:2])
            mypath = osp.join(osp.dirname(osp.dirname(mypath)), 'share',
                              f'twingame-{ver}')
        else:
            # source install
            mypath = osp.join(osp.dirname(mypath), 'share')
        # print('mypath:', mypath)
        return mypath
...
def main():
    ...
    def_config = osp.join(TwinGame.get_share_path(), 'config',
                          'twin_config.json')
